...
(defrule CRITIQUE-PERFORMANCE-PRECALCULATION1::find-list-of-active-instruments
	?ai <- (CRITIQUE-PERFORMANCE-PARAM::list-of-active-instruments (list $?l))
	(DATABASE::Instrument (Name ?instr&:(not-contains$ ?instr $?l)) (Illumination Active))
	=>
	(modify ?ai (list $?l ?instr))
)
